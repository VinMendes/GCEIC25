...
(async () => {
  ...
  try {
    ...
    /* ---------- SOBRE ---------- */
    // 1. clica no botão “Sobre” na Home
    await clickBy(driver, [
    By.css('flt-semantics[role="button"][aria-label="Sobre"]'),
    By.xpath('//*[@aria-label="Sobre"]')
    ]);
    ...
    // 2) Próprio <button> interno
    By.xpath("//button[.//*[normalize-space(text())='Carregar Informações']]"),
    // 3) Qualquer nó com esse texto
    By.xpath("//*[normalize-space(text())='Carregar Informações']")
    ]);

    // 3. dispara a busca de dados
    await btnCarregar.click();
    await pause();                     // pequena animação

    // 4. espera algum RA aparecer (ou o texto “Divisor do Markup” sair)
    await driver.wait(
    until.elementLocated(By.xpath("//*[contains(text(),'RA:')]")),
    TIMEOUT,
    'Dados do Sobre não carregaram'
    );
